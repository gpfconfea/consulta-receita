...
def AP():
    ...
    arquivo = pd.read_csv('app/resources/estados_csv/AP.csv', sep=";")
    arquivo_destino = pd.read_csv('app/resources/sitac_csv/AP.csv', sep=";")

    # Configurações da página de pesquisa
    driver = webdriver.Edge()
    driver.get(
        'https://crea-ap.sitac.com.br/app/view/sight/externo?form=PesquisarProfissionalEmpresa')
        ...
        if colunaSITAC[i] != "Registrada no SITAC":
            ...
            if i > 0 and i % 100 == 0:
                ...
                arquivo_destino.to_csv(
                    'app/resources/sitac_csv/AP.csv', sep=";", index=False)
            resetar_pagina()
            driver.find_element(By.ID, "PJ").click()
            campo_cnpj = driver.find_element(By.ID, "CNPJ")
            botao_pesquisa = driver.find_element(By.ID, "PESQUISAR")
