...
def PE():
    ...
    arquivo = pd.read_csv('app/resources/estados_csv/PE.csv', sep=";")
    arquivo_destino = pd.read_csv('app/resources/sitac_csv/PE.csv', sep=";")
    global data
    data = datetime.now()
    ...
    driver.get(
        'https://crea-pe.sitac.com.br/app/view/sight/externo?form=PesquisarProfissionalEmpresa')
    ...
    for i in range(len(arquivo)):
        ...
        if colunaSITAC[i] != "Registrada no SITAC":
            ...
            if i > 0 and i % 100 == 0:
                ...
                arquivo.to_csv(
                    'app/resources/estados_csv/PE.csv', sep=";", index=False)
                arquivo_destino.to_csv(
                    'app/resources/sitac_csv/PE.csv', sep=";", index=False)
            data = datetime.now()
            data = data.strftime("%d/%m/%Y, %H:%M")
            resetar_pagina()
            driver.find_element(By.ID, "PJ").click()
