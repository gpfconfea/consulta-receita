    ...
    def captura_resultado_pesquisa(i):
        ...
        if not 'Situação do Registro' in driver.page_source:
            ...
            situacao = driver.find_element(
                By.XPATH, "/html/body/div[2]/div/div[4]/div/div[2]/form/div[3]/div[2]/div[1]/div[1]/table/tbody/tr/td[3]").text
            print(situacao)
    ...
    for i in range(len(arquivo)):
        ...
        if colunaSITAC[i] != "Registrada no SITAC":
            ...
            if hora.hour >= 10 and hora.hour < 12:
                while hora.hour >= 10 and hora.hour < 12:
                    time.sleep(1500)
                    hora = datetime.now()
                driver.refresh()
                time.sleep(1)
            ...
            if i > 0 and i % 100 == 0:
                ...
                arquivo.to_csv(
                    'app/resources/estados_csv/MG.csv', sep=";", index=False)
